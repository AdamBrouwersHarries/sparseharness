...
void Logger::setOutput(std::ostream &output) { _output = &output; }

std::ostream &Logger::output() const { return *_output; }

void Logger::setLoggingLevel(Severity::Type severity) { _severity = severity; }
...
std::string severityToString(Logger::Severity::Type severity) {
  switch (severity) {
  case Logger::Severity::LogAlways:
    return "     ";
...
std::string formatHeader(const Logger &logger, Logger::Severity::Type severity,
  ...
  stream << "[" << std::setw(16) << std::setfill('=') << getFileName(file)
         << ":" << std::setw(4) << std::setfill(' ') << std::left << line
         << " ";

#ifndef NPROFILING
  auto sinceStart =
      std::chrono::high_resolution_clock::now() - logger.startTimePoint();
  ...
  stream << std::setw(5) << std::left << severityToString(severity) << "] ";
