...
double getRuntimeInMilliseconds(cl::Event event) {
  ...
  err = clGetEventProfilingInfo(event(), CL_PROFILING_COMMAND_END, sizeof(end),
                                &end, NULL);
  ASSERT(err == CL_SUCCESS);
...
void shutdownExecutor() { executor::terminate(); }
...
double executeKernel(cl::Kernel kernel, int localSize1, int localSize2,
                     int localSize3, int globalSize1, int globalSize2,
                     int globalSize3,
                     const std::vector<executor::KernelArg *> &args) {
  start_timer(executeKernel, Executor);
  ...
  auto event = devPtr->enqueue(
      kernel, cl::NDRange(clGlobalSize1, clGlobalSize2, clGlobalSize3),
...
double execute(const executor::Kernel &kernel, int localSize1, int localSize2,
               int localSize3, int globalSize1, int globalSize2,
               int globalSize3,
               const std::vector<executor::KernelArg *> &args) {
...
void benchmark(const executor::Kernel &kernel, int localSize1, int localSize2,
               int localSize3, int globalSize1, int globalSize2,
               int globalSize3, const std::vector<executor::KernelArg *> &args,
               int iterations, double timeout, std::vector<double> &runtimes) {
  start_timer(benchmark, Executor);
  for (int i = 0; i < iterations; i++) {
    start_timer(benchmark_iteration, Executor);
    ...
    double runtime =
        executeKernel(kernel.build(), localSize1, localSize2, localSize3,
                      globalSize1, globalSize2, globalSize3, args);
...
double evaluate(const executor::Kernel &kernel, int localSize1, int localSize2,
                int localSize3, int globalSize1, int globalSize2,
                int globalSize3, const std::vector<executor::KernelArg *> &args,
  ...
  { // run a single workgroup on dummy data
    auto k = executor::Kernel(
        std::string{"#define WORKGROUP_GUARD {for(int i = 0; i < "
                    "get_work_dim(); ++i) if(get_group_id(i)!=0) return;}\n"} +
            kernel.getSource(),
    ...
    auto wg_size = -1;
    err = openclKernel.getWorkGroupInfo(devPtr->clDevice(),
                                        CL_KERNEL_WORK_GROUP_SIZE, &wg_size);
    if (err != CL_SUCCESS) {
    ...
    err = openclKernel.getWorkGroupInfo(
        devPtr->clDevice(), CL_KERNEL_PRIVATE_MEM_SIZE, &private_mem);
    if (err != CL_SUCCESS) {
      std::cerr << "ERROR " << err << std::endl;
      return -1;
    }
    ...
    auto event = devPtr->enqueue(
        openclKernel, cl::NDRange(clGlobalSize1, clGlobalSize2, clGlobalSize3),
        cl::NDRange(clLocalSize1, clLocalSize2, clLocalSize3));
  ...
  { // actual run
    ...
    for (int i = 0; i < iterations; i++) {
      auto event = devPtr->enqueue(
          openclKernel,
          cl::NDRange(clGlobalSize1, clGlobalSize2, clGlobalSize3),
          cl::NDRange(clLocalSize1, clLocalSize2, clLocalSize3));
    ...
    if (iterations % 2 == 0)
      median =
          (allRuntimes[iterations / 2] + allRuntimes[iterations / 2 - 1]) / 2.0;
    else
