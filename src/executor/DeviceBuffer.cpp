...
cl::Buffer createCLBuffer(const std::shared_ptr<Device> &devicePtr,
                          const size_t size, const size_t elemSize,
                          cl_mem_flags flags) {
  cl::Buffer buffer;
  try {
    buffer =
        cl::Buffer(devicePtr->clContext(), CL_MEM_READ_WRITE, size * elemSize);
  } catch (cl::Error &err) {
...
DeviceBuffer::DeviceBuffer(const std::shared_ptr<Device> &devicePtr,
                           const size_t size, const size_t elemSize,
                           cl_mem_flags flags)
    : _device(devicePtr), _size(size), _elemSize(elemSize), _flags(flags),
...
DeviceBuffer::DeviceBuffer(const DeviceBuffer &rhs)
    : _device(rhs._device), _size(rhs._size), _elemSize(rhs._elemSize),
      _flags(rhs._flags), _buffer() {
...
DeviceBuffer::DeviceBuffer(DeviceBuffer &&rhs)
    : _device(std::move(rhs._device)), _size(std::move(rhs._size)),
      _elemSize(std::move(rhs._elemSize)), _flags(std::move(rhs._flags)),
...
DeviceBuffer &DeviceBuffer::operator=(const DeviceBuffer &rhs) {
  if (this == &rhs)
    return *this; // handle self assignement
  _device = rhs._device;
  _size = rhs._size;
...
DeviceBuffer::~DeviceBuffer() {
  ...
  if (_buffer() != nullptr) {
    ...
    if (refCount > 1) {
      LOG_DEBUG_INFO("OpenCL Buffer object remains alive (Ref count ", refCount,
                     ")");
    }
  }
}

std::shared_ptr<Device> DeviceBuffer::devicePtr() const { return _device; }

DeviceBuffer::size_type DeviceBuffer::size() const { return _size; }

DeviceBuffer::size_type DeviceBuffer::elemSize() const { return _elemSize; }

DeviceBuffer::size_type DeviceBuffer::sizeInBytes() const {
  return _size * _elemSize;
}

const cl::Buffer &DeviceBuffer::clBuffer() const { return _buffer; }

bool DeviceBuffer::isValid() const { return (_buffer() != NULL); }

std::string DeviceBuffer::getInfo() const {
  std::stringstream s;
  s << "device: " << _device->id() << ", size: " << _size
    << ", flags: " << _flags << ", buffer: " << _buffer();
  return s.str();
}
