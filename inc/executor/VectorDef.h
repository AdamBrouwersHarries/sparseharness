...
Vector<T>::Vector(const size_type size, const value_type &value)
    : _size(size), _hostBufferUpToDate(true), _deviceBuffersUpToDate(false),
      _hostBuffer(size, value), _deviceBuffers() {
  (void)registerVectorDeviceFunctions;
  LOG_DEBUG_INFO("Created new Vector object (", this, ") with ",
                 getDebugInfo());
}
...
Vector<T>::Vector(InputIterator first, InputIterator last)
    : _size(std::distance(first, last)), _hostBufferUpToDate(true),
      _deviceBuffersUpToDate(false), _hostBuffer(first, last),
      _deviceBuffers() {
  (void)registerVectorDeviceFunctions;
  LOG_DEBUG_INFO("Created new Vector object (", this, ") with ",
                 getDebugInfo());
...
Vector<T>::Vector(const Vector<T> &rhs)
    : _size(rhs._size), _hostBufferUpToDate(rhs._hostBufferUpToDate),
      _deviceBuffersUpToDate(rhs._deviceBuffersUpToDate),
      _hostBuffer(rhs._hostBuffer), _deviceBuffers(rhs._deviceBuffers) {
  (void)registerVectorDeviceFunctions;
...
template <typename T> Vector<T> &Vector<T>::operator=(Vector<T> &&rhs) {
  ...
  rhs._hostBufferUpToDate = false;
  rhs._deviceBuffersUpToDate = false;
  LOG_DEBUG_INFO("Move assignment to Vector object (", this, ") from (", &rhs,
...
template <typename T> Sizes Vector<T>::sizes() const {
  Sizes s;
#if 0
...
template <typename T> void Vector<T>::reserve(typename Vector<T>::size_type n) {
  return _hostBuffer.reserve(n);
}

template <typename T>
typename Vector<T>::reference Vector<T>::
operator[](typename Vector<T>::size_type n) {
  copyDataToHost();
...
operator[](typename Vector<T>::size_type n) const {
  copyDataToHost();
  return _hostBuffer.operator[](n);
}
...
template <typename T>
typename Vector<T>::iterator
Vector<T>::erase(typename Vector<T>::iterator first,
                 typename Vector<T>::iterator last) {
  _size -= std::distance(first, last);
  return _hostBuffer.erase(first, last);
}

template <typename T> void Vector<T>::swap(Vector<T> &rhs) {
...
template <typename T> void Vector<T>::createDeviceBuffers() const {
...
template <typename T> void Vector<T>::forceCreateDeviceBuffers() const {
  ASSERT(_size > 0);
  ...
  _deviceBuffers.insert(
      {devicePtr->id(), DeviceBuffer(devicePtr, _size, sizeof(T))});
}

template <typename T> Event Vector<T>::startUpload() const {
  ASSERT(_size > 0);
...
template <typename T> void Vector<T>::copyDataToDevices() const {
  if (_hostBufferUpToDate && !_deviceBuffersUpToDate) {
    startUpload().wait();
  }
}

template <typename T> Event Vector<T>::startDownload() const {
  ASSERT(_size > 0);
  ASSERT(!_deviceBuffers.empty());
...
template <typename T> void Vector<T>::copyDataToHost() const {
  if (_deviceBuffersUpToDate && !_hostBufferUpToDate) {
    startDownload().wait();
  }
}

template <typename T> Event Vector<T>::copyDataToHostAsync() const {
  if (_deviceBuffersUpToDate && !_hostBufferUpToDate) {
    return startDownload();
  }
}

template <typename T> void Vector<T>::dataOnDeviceModified() const {
  _hostBufferUpToDate = false;
  _deviceBuffersUpToDate = true;
  LOG_DEBUG_INFO("Data on devices marked as modified");
}

template <typename T> void Vector<T>::dataOnHostModified() const {
  _hostBufferUpToDate = true;
  _deviceBuffersUpToDate = false;
...
template <typename T>
const DeviceBuffer &Vector<T>::deviceBuffer(const Device &device) const {
...
template <typename T> std::string Vector<T>::deviceFunctions() {
...
template <typename T> std::string Vector<T>::getInfo() const {
  std::stringstream s;
  s << "size: " << _size;
  return s.str();
}

template <typename T> std::string Vector<T>::getDebugInfo() const {
  std::stringstream s;
  s << getInfo() << std::boolalpha
